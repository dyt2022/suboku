...
Sudoku.prototype.drawBoard = function(){
        ...
        for (j=0; j < this.nn; j++) {
            ...
            if (value > 0) {
                cell = $('<div></div>')
                            ...
                            .html('<span>'+ value + '</span>' + '<img style="filter: grayscale(30%) brightness(60%)" src="./assets/' + value + '.jpg">');
            } else {
                cell = $('<div></div>')
                            .addClass('cell')
                            .attr('x', position.x)
    ...
    for (i=1; i <= this.nn; i++) {
        $('<div></div>').addClass('num').html(i + '<img src="./assets/' + i + '.jpg">').appendTo(sudoku_console);
    }
...
Sudoku.prototype.resizeWindow = function(){
    ...
    var cell_width = $('#'+ this.id +' .sudoku_board .cell:first').width(),
        ...
        img_width = cell_width - 8;
  
    $('#'+ this.id +' .sudoku_board .cell').css({'position': 'relative', 'height': cell_width+'px'})
    $('#'+ this.id +' .sudoku_board .cell span').css('line-height', cell_width+'px'); 
    $('#'+ this.id +' .sudoku_board .cell img').css({'position': 'absolute', 'top': '4px', 'left': '4px', 'height': img_width+'px'}); 
    ...
    var console_cell_width = $('#'+ this.id +' .board_console .num:first').width(),
        console_img_width = console_cell_width - 4;
    $('#'+ this.id +' .board_console .num').css('height', console_cell_width);
    $('#'+ this.id +' .board_console .num').css('line-height', console_cell_width+'px');
    $('#'+ this.id +' .board_console .num').css('position', 'relative');
    $('#'+ this.id +' .board_console .num img').css({'position': 'absolute', 'top': '2px', 'left': '2px', 'height': console_img_width+'px'});
...
Sudoku.prototype.addValue = function(value) {
    ...
    if (value > 0) {
        $( this.cell ).html('<span>'+ value + '</span>' + '<img src="./assets/' + value + '.jpg"></img>');
    } else {
        $( this.cell ).html('<span></span>');
    }
